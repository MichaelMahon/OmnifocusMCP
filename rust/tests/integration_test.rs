...
async fn test_task_lifecycle() {
    ...
    let fetched = get_task(&runner, &task_id)
        .await
        .expect("get_task should succeed");
    assert_eq!(fetched["id"], task_id, "get_task should return created task");
...
async fn clean_prefix_artifacts() {
...
async fn test_read_tools_return_valid_json() -> Result<()> {
    ...
    let created_project = create_project(
        &runner,
        &cleanup.test_name("Read tool project"),
        None,
        None,
        None,
        None,
