...
async def delete_projects_batch(project_ids_or_names: list[str]) -> str:
    """delete multiple projects by id or name in a single omnijs call."""
    if len(project_ids_or_names) == 0:
        raise ValueError(
            "project_ids_or_names must contain at least one project identifier."
        )

    normalized_identifiers: list[str] = []
    for project_id_or_name in project_ids_or_names:
        normalized_identifier = project_id_or_name.strip()
        if normalized_identifier == "":
            raise ValueError(
                "each project identifier must be a non-empty string."
            )
        normalized_identifiers.append(normalized_identifier)

    identifiers_value = json.dumps(normalized_identifiers)
    script = f"""
const projectIdentifiers = {identifiers_value};
...
@typed_tool(mcp)
async def delete_projects_batch(project_ids_or_names: list[str]) -> str:
    """delete multiple projects by id or exact name in a single omnijs call.

    destructive operation. this permanently removes each matched project and its
    tasks. before calling, always show the user which projects are targeted and
    ...
    normalized_ids_or_names: list[str] = []
    seen_ids_or_names: set[str] = set()
    for project_id_or_name in project_ids_or_names:
        if not isinstance(project_id_or_name, str):
            raise ValueError("each project identifier must be a string.")
        normalized_id_or_name = project_id_or_name.strip()
        if normalized_id_or_name == "":
            raise ValueError("each project identifier must be a non-empty string.")
        if normalized_id_or_name in seen_ids_or_names:
            raise ValueError(
                "project_ids_or_names must not contain duplicates: "
                f"{normalized_id_or_name}"
            )
        seen_ids_or_names.add(normalized_id_or_name)
        normalized_ids_or_names.append(normalized_id_or_name)

    project_ids_or_names_value = json.dumps(normalized_ids_or_names)
    script = f"""
