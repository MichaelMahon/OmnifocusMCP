...
async def add_notification(
    ...
    """add a notification to a task by id.

    provide exactly one of absoluteDate (iso 8601) or relativeOffset (seconds
    relative to the task due date). returns the created notification summary.
    """
    if task_id.strip() == "":
        raise ValueError("task_id must not be empty.")
    if absoluteDate is None and relativeOffset is None:
        raise ValueError("exactly one of absoluteDate or relativeOffset must be provided.")
    ...
    relative_offset_filter = "null" if relativeOffset is None else str(relativeOffset)
    script = f"""
const taskId = {task_id_filter};
