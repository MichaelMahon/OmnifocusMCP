...
`.trim();
  ...
  server.tool(
    ...
    async ({ object_type, object_id, text }) => {
      try {
        if (!["task", "project"].includes(object_type)) {
  ...
  server.tool(
    ...
    async ({ object_type, object_id, text }) => {
      try {
        ...
        const objectTypeValue = escapeForJxa(objectType);
        const objectIdValue = escapeForJxa(objectId);
        const textValue = escapeForJxa(text);
        const script = `
