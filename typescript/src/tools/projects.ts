...
`.trim();
  ...
  server.tool(
    ...
    {
      project_ids_or_names: z.array(z.string()).min(1),
    },
    async ({ project_ids_or_names }) => {
      try {
        ...
        const projectIdsOrNamesValue = JSON.stringify(normalizedIdsOrNames);
        const script = `
  ...
  server.tool(
    ...
    "delete multiple projects by id or exact name in a single omnijs call.",
    {
      project_ids_or_names: z
        ...
        .min(1)
        .describe("required non-empty array of project ids or exact names"),
    },
    async ({ project_ids_or_names }) => {
      try {
        if (project_ids_or_names.length === 0) {
        ...
        const normalizedIdentifiers = project_ids_or_names.map((identifier) => {
        ...
        const script = `
const projectIdentifiers = ${identifiersValue};
const projectById = new Map();
const projectByName = new Map();
for (const project of document.flattenedProjects) {
  try {
